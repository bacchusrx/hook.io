...
var dnode = require('../../vendor/dnode'),
    ...
    EventEmitter = require('../../vendor/EventEmitter2/lib/eventemitter2').EventEmitter2;
...
Hook.prototype.kill = function(hooks, callback) {
  ...
  if (typeof hooks === 'string') {
    ...
    if (self.children[hooks] && self.children[hooks].monitor) {
      ...
      delete self.children[hooks];
      delete self._names[hooks];
      callback(null);
    } else {
...
Hook.prototype.listen = function (options, callback) {
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    try {
      if (self['hook-socket']) {
        self.server.listen(path.resolve(self['hook-socket']));
      }
      else {
...
Hook.prototype._cliOptions = function (options) {
  ...
  //
  var reserved_cli = ['port', 'host', 'socket', 'name', 'type'];
