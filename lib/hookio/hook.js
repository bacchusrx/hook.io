...
Hook.prototype.listen = function (options, callback) { 
  ...
  this['hook-socket'] = options['hook-socket'] || this.defaults['hook-socket'];
  ...
  self.toIPs(self['hook-host'], function onResolve (err, hosts) {
    ...
    self.server.on('error', function (err) {
      if (err) {
        if (err.code == 'EADDRINUSE') {
          self.emit('error::bind', self['hook-socket'] || self['hook-port']);
    ...
    self.server.on('ready', function () {
      ...
      self.emit('hook::listening', self['hook-socket'] || self['hook-port']);
      self.emit('hook::started', self['hook-socket'] || self['hook-port']);
    ...
    try {
      if (self['hook-socket']) {
        var socketPath = path.resolve(self['hook-socket']);

        // TODO: There's probably a better way to do this. This is hacking
        // around an issue in libuv. See:
        //
        // https://github.com/joyent/libuv/issues/158. 
        //
        // Basically, libuv allows multiple binds to the same pipe, which 
        // breaks `start`, where we attempt to `listen` and fall back on
        // `connect` if there's a bind error. Since there won't be a bind
        // error, each Hook that starts ends up stealing the pipe from the
        // last.
       
        // XXX What about stale sockets?
        if (path.existsSync(socketPath)) {
          self.server.emit('error', {code: 'EADDRINUSE'});
        }
        else {
          self.server.listen(socketPath);
        }
      }
      else {
...
Hook.prototype.connect = function (options, callback) {
  ...
  this['hook-socket'] = options['hook-socket'] || this['hook-socket'] || this.defaults['hook-socket'];
  ...
  client.connect(dnodeOptions, function (remote, conn) {
    ...
    remote.report(self.getInfo(),  function (newName, newID, pkginfo) {
      ...
      self.emit('hook::connected', self['hook-socket'] || self['hook-port']);
      self.emit('hook::started', self['hook-socket'] || self['hook-port']);
...
Hook.prototype._dnodeOptions = function () {
  ...
  if (this['hook-socket']) {
    opts.path = path.resolve(this['hook-socket']);
  }
  else {
    opts.host = this['hook-host'];
    opts.port = this['hook-port'];
  }
