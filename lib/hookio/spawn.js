...
exports.spawn = function (hooks, callback) {
  ...
  function spawnHook (hook, next) {
    ...
    if (hook['socket'] || self['hook-socket']) {
      hook['socket'] = hook['socket'] || self['hook-socket'];
    } else {
    ...
    if (local) {
      ...
      // 
      // XXX This is how forever.Monitor determines whether we're executing node
      // with a script or some other command. Since our custom forever shim is
      // a compiled executable, we have to supply the executable name and any
      // CLI options as an array.
      //
